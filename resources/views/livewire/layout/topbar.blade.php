...
<div class="flex items-center gap-4">
    ...
    <div x-data="{ open: false }" class="relative">
        <button @click="open = !open" @click.outside="open = false" class="relative flex items-center justify-center w-10 h-10 text-slate-700 hover:text-brand-600 hover:bg-brand-50 rounded-xl transition-colors" title="Notifications">
        ...
        <div x-show="open" x-transition.opacity style="display: none;" class="absolute right-0 mt-2 w-96 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden">
            <div class="px-4 py-3 bg-slate-50 border-b border-slate-100 flex justify-between items-center gap-4">
                ...
                @if(auth()->user()->unreadNotifications->count() > 0)
                    <button wire:click="markAllRead" class="text-xs text-brand-600 hover:text-brand-800 font-semibold whitespace-nowrap transition-colors">Mark all as read</button>
                @endif
            </div>
    ...
    <!-- Profile -->
    <a href="{{ route('profile') }}" wire:navigate class="flex items-center gap-2 px-3 py-2 text-sm font-semibold text-slate-700 hover:text-brand-600 hover:bg-brand-50 rounded-xl transition-colors">
